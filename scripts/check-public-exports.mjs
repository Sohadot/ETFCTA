...
for(const path of ['data/changes.json','data/review-calendar.json','data/evidence-status.json','data/methodology-gaps.json']){const value=JSON.parse(await readFile(path,'utf8'));const errors=findPublicExportLeaks(value);if(errors.length){console.error(`${path}\n${errors.join('\n')}`);process.exit(1)}}
...
console.log(`PASS public export boundary (${paths.length + 5} exports, ${forbiddenSamples.length} negative tests)`);
