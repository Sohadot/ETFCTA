...
if(!process.env.GAL_PRE_ATTESTATION) {
  ...
  const releaseResult=releaseGovernanceGate({contract,releaseGate,routeManifest,changelog:await readFile('CHANGELOG.md','utf8'),decisionLog:await readFile('governance/DECISION_LOG.md','utf8'),releaseArchive,attestation,artifactHashes,integrityManifestHash,subjectCommitSha:commit_sha});
