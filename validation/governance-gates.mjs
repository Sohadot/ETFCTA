...
export function evidenceGovernanceGate({registry,externalLinks,now='2026-07-18'}) {
  const violations=[]; const sources=new Map(registry.sources.map(x=>[x.source_id,x])); const evidence=new Map(registry.evidence.map(x=>[x.evidence_id,x]));
  for(const source of registry.sources) {
    if(!source.source_id||!source.authority||!source.locator) violations.push(violation('EVS-R01',source.source_id??'unknown','source identity incomplete'));
    if(source.preservation?.status==='preserved'&&(!/^[a-f0-9]{64}$/.test(source.document_sha256??'')||source.preservation.document_sha256!==source.document_sha256)) violations.push(violation('EVS-R01',source.source_id,'source and preservation SHA-256 mismatch'));
    if(source.preservation?.status==='preservation_unavailable'&&(source.preservation.document_sha256!==null||!source.preservation.limitation)) violations.push(violation('EVS-R01',source.source_id,'unavailable preservation requires null hash and limitation'));
    if(source.preservation?.status==='not_preserved') violations.push(violation('EVS-R01',source.source_id,'source is not preserved'));
    const age=(Date.parse(`${now}T00:00:00Z`)-Date.parse(`${source.retrieved_at}T00:00:00Z`))/86400000; if(!Number.isFinite(age)||age>365) violations.push(violation('EVS-R04',source.source_id,'source retrieval is outside the annual freshness window'));
    if(source.verification_status==='superseded'&&!source.supersession_justification) violations.push(violation('EVS-R03',source.source_id,'superseded source lacks justification'));
  }
  for(const record of registry.evidence) { const source=sources.get(record.source_id); if(record.admission?.state==='admitted'&&source?.verification_status!=='verified') violations.push(violation('EVS-R02',record.evidence_id,'admitted evidence source is not verified')); }
  for(const decision of registry.decisions.filter(x=>['confirmed','provisional'].includes(x.decision_status))) { const supporting=decision.evidence_ids.map(id=>evidence.get(id)).filter(Boolean).map(x=>sources.get(x.source_id)).filter(Boolean); if(supporting.length&&supporting.every(x=>x.verification_status==='withdrawn')) violations.push(violation('EVS-R03',decision.decision_id,'current decision is supported only by withdrawn sources')); }
  const observations=new Map((externalLinks?.observations??[]).map(x=>[x.source_id,x]));
  for(const source of registry.sources.filter(x=>x.locator?.url)) { const observed=observations.get(source.source_id); if(!observed||observed.status==='not_checked') violations.push(violation('EVS-R06',source.source_id,'external link has no current observation')); else if(observed.status==='dead') violations.push(violation('EVS-R06',source.source_id,'external link is dead')); if(!observed?.newer_version_status) violations.push(violation('EVS-R04',source.source_id,'newer-version state is not recorded')); }
  const primaryTypes=new Set(['primary_fund_document','issuer_fund_page','official_holdings_csv','sec_filing_metadata','official_fact_sheet']); let primaryClaims=0,compliant=0;
  for(const decision of registry.decisions) { primaryClaims++; const records=decision.evidence_ids.map(id=>evidence.get(id)).filter(Boolean); if(records.length&&records.every(x=>primaryTypes.has(sources.get(x.source_id)?.source_type))) compliant++; }
  return {violations,metrics:[metric('hash_compliance_pct',(registry.sources.length-violations.filter(v=>v.rule_id==='EVS-R01').length)/registry.sources.length*100,'percent',100,{denominator:registry.sources.length}),metric('freshness_compliance_pct',(registry.sources.length-violations.filter(v=>v.rule_id==='EVS-R04').length)/registry.sources.length*100,'percent',100,{denominator:registry.sources.length}),metric('identified_sources_pct',(registry.sources.length-violations.filter(v=>v.rule_id==='EVS-R02').length)/registry.sources.length*100,'percent',100,{denominator:registry.sources.length}),metric('primary_claim_source_compliance_pct',primaryClaims?compliant/primaryClaims*100:100,'percent',100,{numerator:compliant,denominator:primaryClaims})]};
}

export function securitySupplyChainGate({files,publicFiles,packageJson,policy,licenses,integrityFailures=[],workflows=[]}) {
  const violations=[]; const secretPatterns=[/AKIA[0-9A-Z]{16}/,/gh[pousr]_[A-Za-z0-9_]{30,}/,/sk-[A-Za-z0-9]{20,}/,/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/,/(?:password|api_key|token)\s*[=:]\s*["'][^"']{8,}["']/i];
  for(const [file,body] of Object.entries(files)) for(const pattern of secretPatterns) if(pattern.test(body)) violations.push(violation('SSS-R01',file,'secret-like material detected'));
  for(const [file,body] of Object.entries(publicFiles)) if(/[A-Z]:\\(?:Users|Projects)\\|\/Users\/|"preservation_locator"\s*:\s*"(?!sha256:)/i.test(body)) violations.push(violation('SSS-R01',file,'local path or internal preservation locator in public output'));
  const dependencies={...(packageJson.dependencies??{}),...(packageJson.devDependencies??{})}; for(const name of Object.keys(dependencies)) if(!policy.allowed_package_dependencies.includes(name)) violations.push(violation('SSS-R02',name,'unapproved package dependency'));
  const requiredCategories=['repository_code','ecs_specification','ontology_and_governance','published_data','third_party_excerpts','images_and_fonts']; for(const category of requiredCategories) { const item=licenses.categories.find(x=>x.category===category); if(!item||!item.license_status||item.license_status==='unknown') violations.push(violation('SSS-R03',category,'license status missing or unknown')); }
  for(const file of integrityFailures) violations.push(violation('SSS-R04',file,'governed file hash mismatch'));
  for(const [file,body] of workflows) { for(const use of body.matchAll(/uses:\s*([^\s]+)/g)) if(!/@[a-f0-9]{40}$/i.test(use[1])) violations.push(violation('SSS-R05',file,`unpinned action ${use[1]}`)); if(!/^permissions:\s*contents:\s*read/m.test(body)&&!/permissions:\s*\n\s*contents:\s*read/m.test(body)) violations.push(violation('SSS-R05',file,'workflow minimum permissions not declared')); }
  return {violations,metrics:[metric('known_vulnerabilities_count',0,'count',0,{status:Object.keys(dependencies).length?'not_measured':'not_applicable_no_dependencies'}),metric('detected_secrets_count',violations.filter(v=>v.rule_id==='SSS-R01').length,'count',0),metric('unapproved_licenses_count',violations.filter(v=>v.rule_id==='SSS-R03').length,'count',0),metric('reference_hash_failures_count',violations.filter(v=>v.rule_id==='SSS-R04').length,'count',0),metric('commit_signature_status',null,'state','verified',{status:'not_measured_non_blocking'})]};
}

export function releaseGovernanceGate({contract,releaseGate,routeManifest,changelog,decisionLog,releaseArchive,attestation,artifactHashes,integrityManifestHash,subjectCommitSha,verifyArtifactHashes=true}) {
  const violations=[]; const enforced=new Set(contract.gates.filter(x=>x.state==='enforced').map(x=>x.id)); for(const id of contract.gate_order) if(!enforced.has(id)) violations.push(violation('RGS-R01',id,'gate is not enforced'));
  if(!changelog.includes(`site release ${routeManifest.site_release}`)&&!changelog.includes(`Site ${routeManifest.site_release}`)) violations.push(violation('RGS-R03','CHANGELOG.md','current site release missing from changelog'));
  if(!decisionLog.includes('DEC-ETFCTA-020')) violations.push(violation('RGS-R03','governance/DECISION_LOG.md','Sprint 4D decision missing'));
  if(!releaseArchive) violations.push(violation('RGS-R03',`releases/site-${routeManifest.site_release}.md`,'release archive missing'));
  if(releaseGate.governance_automation_contract!==`${contract.contract_id}-v${contract.version}`) violations.push(violation('RGS-R02','protocol/release-gate.json','GAL version mismatch'));
  if(releaseGate.required_gal_gates?.length!==9) violations.push(violation('RGS-R01','protocol/release-gate.json','nine required GAL gates not declared'));
  if(!attestation) violations.push(violation('RGS-R03',`site-${routeManifest.site_release}`,'attestation missing'));
  else { if(attestation.site_release!==routeManifest.site_release||attestation.data_release!==routeManifest.data_release) violations.push(violation('RGS-R02','attestation','release versions mismatch')); if(attestation.subject_commit_sha!==subjectCommitSha) violations.push(violation('RGS-R04','attestation','subject commit SHA mismatch')); if(attestation.gal_contract!==`${contract.contract_id}-v${contract.version}`) violations.push(violation('RGS-R02','attestation','GAL version mismatch')); const requiredVersions=['validator','schemas','terms','claims','relationships','evidence_standard','security_standard','release_standard']; for(const key of requiredVersions) if(!attestation.versions?.[key]) violations.push(violation('RGS-R02','attestation',`missing version: ${key}`)); if(attestation.integrity_manifest_sha256!==integrityManifestHash) violations.push(violation('RGS-R04','attestation','integrity manifest hash mismatch')); if(verifyArtifactHashes) for(const [gate,hash] of Object.entries(attestation.artifact_hashes??{})) if(artifactHashes[gate]!==hash) violations.push(violation('RGS-R04',gate,'artifact hash mismatch')); for(const id of contract.gate_order) if(attestation.gate_results?.[id]!=='pass') violations.push(violation('RGS-R05',id,'attested gate is not pass')); }
  return {violations,metrics:[metric('blocking_gates_passed_pct',violations.some(v=>v.rule_id==='RGS-R01'||v.rule_id==='RGS-R05')?0:100,'percent',100),metric('version_mismatches_count',violations.filter(v=>v.rule_id==='RGS-R02').length,'count',0),metric('missing_release_artifacts_count',violations.filter(v=>v.rule_id==='RGS-R03'||v.rule_id==='RGS-R04').length,'count',0)]};
}
